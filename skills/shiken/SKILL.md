---
name: shiken
description: "TDD discipline: write failing test first, witness it fail, implement minimally, and PRUNE tests after green"
...
発話トリガーは明示 opt-in 用、kouchiku 計画実行モードとの競合を防ぐ。
...
| **PRUNE** | 各 test を以下基準で評価し、不要なものを削除 |
...
# PRUNE 後、実装 revert で test fail 目視 (上記「PRUNE 検証手順」)
# sadoku レビュー → main へ merge
```

worktree 内では「捨てる」コストがゼロ。実装ロックな test を書いても `delete = delete` で問題ない。本流 branch を汚さない。
...
cycle:             RED -> GREEN -> REFACTOR -> PRUNE 各 phase 完了確認
                     ...
                     検証ログ (PRUNE):   各 test に対する stash → fail → pop → pass の出力
