...
- `wt` — git worktree shell tool (実装済 v0.1.0、`bin/wt help` で詳細、judgment 24 で WT-SPEC.md は廃止)
...
1. **選択肢を提示する**: open question (「どうしますか?」) を投げず、案 + 推奨を出す。ただし**明らかな場合は 1 案でよい** (`kouchiku` の「3 案以上は出さない」と整合、迷い時のみ 2-3 案)
...
- 「明らかなら 1 案、迷い時のみ 2-3 案」(無用な選択肢水増しを防ぐ、`kouchiku` paralysis 防止と整合)
...
| 2. **Perplexity** (検索特化、引用付き) | 複数ソース統合、cross-check | 引用 link 必須 |
| 3. **Grok** (リアルタイム、X 統合) | 最新動向、リアルタイム情報 | 引用 link 必須 |
| 4. WebSearch (ハーネス標準ツール) | 一次ソース URL 発掘の入口 | 検索結果から 1 へ |
...
**実例 (このリポジトリで実際に起きた失敗)**: 2026/05 の kufuu 設計時、私 (Claude) は Anthropic の Agent Skills 標準 (2025/12 release) を「fabricated 疑い」と誤判断した。WebSearch / WebFetch で先に裏取りすれば防げた失敗。判断 23 はこの実例から立てた。
...
- **選択肢の水増し**: 「明らかなら 1 案、迷い時のみ 2-3 案」、`kouchiku` の paralysis 防止 (判断 14) と整合
...
**結論**: kufuu は Anthropic 提唱の **Agent Skills 標準** (2025/12/18 release、agentskills.io) に準拠する。判断 20 (adapters/ 階層) は廃止、`skills/` を SoT として保持しつつ `npx skills add` 経由で全ハーネスに自動配置できる構造にする。

**経緯**:
- 判断 20 を立てた時点 (2026/05) では「ハーネス独自仕様の matrix」を前提に adapters/ で変換する設計を考えていた
- しかし実際は Anthropic が 2025/12 に Agent Skills 標準を release、Cursor 2.4 / Codex / Gemini CLI / Goose / OpenHands ほか 30+ ツールが対応済
...
**標準準拠で得られたもの**:
- 30+ AI tool に対応 (Claude Code / Cursor / Codex / Gemini CLI / Goose / OpenHands / GitHub Copilot 他)
- `npx skills add github:hayashiii-ghub/kufuu` の 1 コマンドで配置可能
- adapter メンテコストゼロ
- 配布性向上 (Agensi Marketplace 等への公開も技術的に可能)
...
**結論**: AI agent の知識カットオフ後 / 不確実な事実は、Perplexity / Grok / WebSearch / 一次ソース fetch で**裏取りしてから断定する**。kufuu の skill / DESIGN 全体に Hard Rule として組み込む (§3.10)。

**直接のきっかけ (実害例)**:
- 2026/05、Cursor 2.4 の skills 機能 / Anthropic Agent Skills 標準 / Agensi Marketplace を私 (Claude) は「fabricated 疑い」と推定で否定した
- 実際は全部実在 (2025/12 release)、私のカットオフ (2026/01) 直後の事実だった
- WebSearch を最初にしていれば防げた誤判断、kufuu 設計が一時的に誤った方向 (judgment 20 の adapters/ 構造) に進んだ

**裏取り手段の優先順** (§3.10):
1. 一次ソース直接 fetch (公式 docs / GitHub / changelog)
2. Perplexity (検索特化、引用付き)
3. Grok (リアルタイム、X 統合)
4. WebSearch (一次ソース URL の入口)
...
**残した 2 ファイルの役割分担**:
- `DESIGN.md`: 設計判断 24 件、開発者 (kufuu を改良したい人) 向け
...
| `~/.claude/skills/` への deploy | 完 | judgment 21 で `npx skills add` 経由に統一、README に手順 |
| `wt` shell tool 実装 | 完 | v0.1.0 実装済 (`bin/wt`、391 行 bash)、smoke test 通過 |
| HTML 配布物の整理 | 完 | judgment 24 で workflow.html / WT-SPEC.md 削除、docs は DESIGN.md + workflow.md の 2 つに |
| 実 deploy で試運転 | 未 | `npx skills add github:hayashiii-ghub/kufuu` で実 task で動かす |
| factcheck 原則の運用検証 | 未 | Perplexity / Grok の API 接続方法 (MCP server か Web 経由か) を実利用で詰める |
